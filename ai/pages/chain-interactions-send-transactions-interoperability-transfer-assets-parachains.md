...
This guide walks you through transferring tokens between two parachains using the [ParaSpell XCM SDK](/polkadot-mkdocs/reference/tools/paraspell/){target=\_blank}. This example utilizes [Asset Hub](/polkadot-mkdocs/reference/polkadot-hub/#asset-management){target=\_blank} and the [People Chain](/polkadot-mkdocs/reference/polkadot-hub/#people-chain){target=\_blank}. However, the same approach can be applied to transfers between other parachains.

For development purposes, this guide will use the [Polkadot TestNet](/polkadot-mkdocs/smart-contracts/connect/#networks-details){target=\_blank}, so the transferred token will be PAS.

In this guide, you will:

- Build an XCM transfer transaction using ParaSpell XCM SDK.
- Perform a dry run to validate the transfer.
- Verify the [Existential Deposit (ED)](/polkadot-mkdocs/reference/glossary/#existential-deposit){target=\_blank} requirement on the destination chain.
...
- Knowledge of the [fundamentals of Polkadot](/polkadot-mkdocs/parachains/get-started/){target=\_blank}.
- Basic understanding of [XCM](/polkadot-mkdocs/parachains/interoperability/get-started/){target=\_blank}.
...
```bash
npm install --save @paraspell/sdk@12.3.0 polkadot-api@1.23.2 @polkadot-labs/hdkd-helpers@0.0.27 @polkadot-labs/hdkd@0.0.26
```
...
Check if the recipient account meets the [Existential Deposit (ED)](/polkadot-mkdocs/reference/glossary/#existential-deposit){target=\_blank} requirement before sending by using [`verifyEdOnDestination`](https://paraspell.github.io/docs/sdk/xcmUtils.html#verify-ed-on-destination){target=\_blank}:
