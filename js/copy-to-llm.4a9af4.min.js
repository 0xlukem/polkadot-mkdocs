...
function buildSlugFromPath(pathname,toggleFilename){const route=(pathname||'').replace(/^\/+|\/+$/g,'');if(toggleFilename){return(route.split('/').filter(Boolean).slice(0,-1).join('-')+
`-${toggleFilename}`);}else{return route.split('/').filter(Boolean).join('-');}}
function getMarkdownUrl(slug){return`${window.location.origin}/ai/pages/${slug}.md`;}
...
function addSectionCopyButtons(mainTitle,toggleFilename){if(mainTitle){const wrapper=document.createElement('div');wrapper.className='h1-copy-wrapper';mainTitle.parentNode.insertBefore(wrapper,mainTitle);wrapper.appendChild(mainTitle);const{container,copyButton,dropdownButton,dropdownMenu}=createSectionCopyButton();[copyButton,dropdownButton].forEach((button)=>{button.addEventListener('keydown',(e)=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();button.click();}});});dropdownMenu.addEventListener('keydown',(e)=>{const items=Array.from(dropdownMenu.querySelectorAll('.copy-to-llm-dropdown-item'));const currentIndex=items.findIndex((item)=>item===document.activeElement);switch(e.key){case'ArrowDown':e.preventDefault();if(!items.length)break;items[(currentIndex+1)%items.length].focus();break;case'ArrowUp':e.preventDefault();if(!items.length)break;items[(currentIndex-1+items.length)%items.length].focus();break;case'Escape':e.preventDefault();dropdownMenu.classList.remove('show');dropdownButton.classList.remove('active');dropdownButton.setAttribute('aria-expanded','false');dropdownButton.focus();resetChevron(dropdownButton);break;}});copyButton.addEventListener('click',async(event)=>{event.preventDefault();trackButtonClick('copy_page_markdown');const icon=copyButton.querySelector('.copy-icon');const originalIconHTML=icon?icon.outerHTML:'';if(icon){icon.outerHTML=`
          ...
          `;}
let copySucceeded=false;const slug=buildSlugFromPath(window.location.pathname,toggleFilename);const{text,status}=await fetchMarkdown(slug);if(text){copySucceeded=await copyToClipboard(text,copyButton,'markdown_content');}
else{if(status===404){showToast(NO_MARKDOWN_MESSAGE);}}
...
const action=item.dataset.action;const slug=buildSlugFromPath(window.location.pathname,toggleFilename);switch(action){case'view-markdown':{trackButtonClick('view_page_markdown');const mdUrl=getMarkdownUrl(slug);window.open(mdUrl,'_blank','noopener,noreferrer');break;}
...
function initialize(){if(document.querySelector('h1.not-found')){return;}
const toggleContainers=document.querySelectorAll('.toggle-container');if(toggleContainers.length===0){const title=document.querySelector('.md-content h1');if(title){addSectionCopyButtons(title,null);}
return;}
toggleContainers.forEach((container)=>{const headerVariants=container.querySelectorAll('.toggle-header > span[data-variant]');headerVariants.forEach((headerSpan)=>{const h1=headerSpan.querySelector('h1');if(!h1)return;const variant=headerSpan.dataset.variant;const button=container.querySelector(`.toggle-btn[data-variant="${variant}"]`);const toggleFilename=button?.dataset.filename||null;addSectionCopyButtons(h1,toggleFilename);});});}
